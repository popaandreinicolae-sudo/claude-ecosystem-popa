...
export class HudocClient {
  ...
  buildQuery(params: {
    ...
    if (params.freeText) parts.push(`"${params.freeText}"`);
    if (params.appno) parts.push(`appno:"${params.appno}"`);
    if (params.article) parts.push(`article:"${params.article}"`);
    if (params.respondent) parts.push(`respondent:"${params.respondent}"`);
    if (params.dateFrom) parts.push(`judgmentdate>="${params.dateFrom}"`);
    if (params.dateTo) parts.push(`judgmentdate<="${params.dateTo}"`);
    if (params.violation && params.violation !== "all") parts.push(`violation:"${params.violation.toUpperCase()}"`);
    if (params.importance && params.importance !== "all") parts.push(`importance:"${params.importance}"`);
    if (params.collection && params.collection !== "all") parts.push(`documentcollectionid2:"${params.collection.toUpperCase()}"`);
  ...
  async searchCases(query: string, start: number = 0, length: number = 20): Promise<{ results: Array<Record<string, unknown>>; total: number }> {
    ...
    return this.retry(async () => {
      const resp = await this.client.get<HudocResult>(HUDOC_SEARCH, {
        params: {
          query,
          ...
          start,
          length,
          rankingModelId: "11111111-1111-1111-1111-111111111111",
        },
      });
  ...
  private val(r: Record<string, string | string[]>, key: string): string {
    const v = r[key];
